...
public ResponseEntity<?> layKhungGioDaDat(
    ...
    /**
 * POST /api/dat-san/hold-slot
 * Giữ chỗ tạm thời (5 phút đếm ngược)
 */
...
public ResponseEntity<?> holdSlot(
    ...
    try {
        String email = layEmailTuToken(request);
        String clientIp = layClientIp(request);
        DatSanPhanHoi ketQua = datSanService.holdSlot(yeuCau, email, clientIp);
        return ResponseEntity.ok(ketQua);
    } catch (RuntimeException e) {
        return ResponseEntity.badRequest().body(Map.of("message", e.getMessage()));
    }
...
/**
 * POST /api/dat-san/{id}/cancel-hold
 * Hủy giữ chỗ lập tức khi bấm Quay lại hoặc đóng tab (beacons)
 */
...
public ResponseEntity<?> cancelHold(
    ...
    try {
        String email = layEmailTuToken(request);
        String ketQua = datSanService.cancelHold(id, email);
        return ResponseEntity.ok(Map.of("message", ketQua));
    } catch (RuntimeException e) {
        return ResponseEntity.badRequest().body(Map.of("message", e.getMessage()));
    }
...
// Helper lấy IP Khách hàng
private String layClientIp(HttpServletRequest request) {
