...
IF EXISTS (
    SELECT name
    FROM sys.databases
    WHERE name = N'QuanLySanBong'
)
BEGIN
    ALTER DATABASE QuanLySanBong SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
...
ALTER AUTHORIZATION ON DATABASE::QuanLySanBong TO sa;
GO
...
-- Đổi giá trị mặc định của trang_thai từ 'THANH_CONG' sang 'DANG_CHO'.
-- KHÔNG hardcode tên constraint: SQL Server tự sinh tên kiểu
-- DF__THANH_TOA__trang__XXXXXXXX với hậu tố ngẫu nhiên KHÁC NHAU trên mỗi máy,
-- nên phải tra tên thật trong sys.default_constraints rồi mới xoá.
DECLARE @tenConstraint SYSNAME;
SELECT @tenConstraint = dc.name
FROM sys.default_constraints dc
JOIN sys.columns c ON dc.parent_object_id = c.object_id
                  AND dc.parent_column_id = c.column_id
WHERE dc.parent_object_id = OBJECT_ID('THANH_TOAN')
  AND c.name = 'trang_thai';

IF @tenConstraint IS NOT NULL AND @tenConstraint <> 'DF_THANH_TOAN_TRANG_THAI'
    EXEC('ALTER TABLE THANH_TOAN DROP CONSTRAINT [' + @tenConstraint + ']');

IF NOT EXISTS (SELECT 1 FROM sys.default_constraints WHERE name = 'DF_THANH_TOAN_TRANG_THAI')
