      ...
      <div class="modal-box">
        ...
        <div class="modal-body">
          <div class="info-block">
            ...
            <div class="info-row highlight"><span class="info-label">Cần thu nốt:</span><span class="price-big">{{ formatMoney(tinhConLai()) }}</span></div>
          </div>
...
function openThuNot(tt) {
  thuNotModal.value = { show: true, tt, phuongThuc: 'TIEN_MAT', loading: false }
}

function tinhConLai() {
  if (!thuNotModal.value.tt) return 0
  // Tìm đơn gốc để tính: tongTien - tienCoc (tt.soTien là tiền cọc)
  const tt = thuNotModal.value.tt
  // API sẽ tự tính, hiển thị ước tính từ dữ liệu có
  return tt.soTien || 0
}
...
.info-row { display: flex; gap: 12px; align-items: center; padding: 7px 0; border-bottom: 1px solid #f1f5f9; }
