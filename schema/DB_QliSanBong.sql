...
-- Index tăng tốc tìm kiếm theo Slug và Status
CREATE INDEX IX_Posts_Slug ON Posts(slug);
CREATE INDEX IX_Posts_Status_CreatedAt ON Posts(status, created_at DESC);
...
ALTER TABLE DAT_SAN ADD expire_at DATETIME NULL, ip_address VARCHAR(45) NULL;
...
-- Bảng quản lý voucher do Admin tạo và quản lý
CREATE TABLE VOUCHER (
    ...
    loai_giam VARCHAR(20) NOT NULL
        CHECK (loai_giam IN ('PHAN_TRAM', 'CO_DINH')),
    ...
    don_toi_thieu DECIMAL(18,2) NULL
        CHECK (don_toi_thieu IS NULL OR don_toi_thieu >= 0),

    -- ALL      : tất cả khách hàng
    -- NEW_USER : khách chưa có lượt đặt thành công trong 30 ngày
    -- SILVER   : từ 3 lượt đặt thành công / 30 ngày
    -- GOLD     : từ 6 lượt đặt thành công / 30 ngày
    -- DIAMOND  : từ 10 lượt đặt thành công / 30 ngày
    doi_tuong_khach_hang VARCHAR(20) NOT NULL DEFAULT 'ALL'
        CHECK (doi_tuong_khach_hang IN ('ALL','NEW_USER','SILVER','GOLD','DIAMOND')),

    so_luong INT NOT NULL CHECK (so_luong > 0),
    da_su_dung INT NOT NULL DEFAULT 0 CHECK (da_su_dung >= 0),
    gioi_han_moi_khach INT NOT NULL DEFAULT 1 CHECK (gioi_han_moi_khach > 0),

    ngay_bat_dau DATETIME NOT NULL,
    ngay_ket_thuc DATETIME NOT NULL,
    trang_thai VARCHAR(20) NOT NULL DEFAULT 'HOAT_DONG'
        CHECK (trang_thai IN ('HOAT_DONG','TAM_DUNG','HET_HAN')),

    CONSTRAINT CK_VOUCHER_THOI_GIAN CHECK (ngay_ket_thuc > ngay_bat_dau),
    CONSTRAINT CK_VOUCHER_SU_DUNG CHECK (da_su_dung <= so_luong),
    CONSTRAINT CK_VOUCHER_PHAN_TRAM CHECK (
        loai_giam <> 'PHAN_TRAM' OR gia_tri_giam <= 100
    )
);
GO

-- Lưu voucher đã áp dụng và số tiền giảm ngay trên đơn đặt sân
ALTER TABLE DAT_SAN ADD voucher_id UNIQUEIDENTIFIER NULL, tien_giam_voucher DECIMAL(18,2) NULL DEFAULT 0;
ALTER TABLE DAT_SAN ADD CONSTRAINT FK_DAT_SAN_VOUCHER FOREIGN KEY (voucher_id) REFERENCES VOUCHER(id);
GO

-- Bảng lưu lịch sử mỗi lần khách thực sự sử dụng voucher
CREATE TABLE SU_DUNG_VOUCHER (
    ...
    CONSTRAINT FK_SDV_VOUCHER
        FOREIGN KEY (voucher_id) REFERENCES VOUCHER(id),
    CONSTRAINT FK_SDV_USER
        FOREIGN KEY (nguoi_dung_id) REFERENCES USERS(id),
    CONSTRAINT FK_SDV_DAT_SAN
        FOREIGN KEY (dat_san_id) REFERENCES DAT_SAN(id),
    CONSTRAINT UQ_SDV_VOUCHER_USER_DAT_SAN
        UNIQUE (voucher_id, nguoi_dung_id, dat_san_id)
);
GO

-- Index phục vụ kiểm tra số lượt đặt sân thành công trong 30 ngày
CREATE INDEX IX_DAT_SAN_HANG_VOUCHER
ON DAT_SAN (nguoi_dung_id, trang_thai, ngay_dat);
GO

-- Index phục vụ tìm voucher theo mã, trạng thái và thời gian hiệu lực
CREATE INDEX IX_VOUCHER_TRANG_THAI_THOI_GIAN
ON VOUCHER (trang_thai, ngay_bat_dau, ngay_ket_thuc);
GO

-- Index phục vụ kiểm tra lịch sử sử dụng voucher của từng khách
CREATE INDEX IX_SDV_NGUOI_DUNG
ON SU_DUNG_VOUCHER (nguoi_dung_id, voucher_id, ngay_su_dung);
GO

-- ==========================================
-- DỮ LIỆU VOUCHER MẪU (có thể xóa nếu không cần)
-- ==========================================
INSERT INTO VOUCHER
(ma, ten, mo_ta, loai_giam, gia_tri_giam, giam_toi_da, don_toi_thieu,
 doi_tuong_khach_hang, so_luong, da_su_dung, gioi_han_moi_khach,
 ngay_bat_dau, ngay_ket_thuc, trang_thai)
VALUES
('WELCOME20', N'Ưu đãi khách mới', N'Giảm 20.000đ cho khách mới',
 'CO_DINH', 20000, NULL, 100000, 'NEW_USER', 100, 0, 1,
 GETDATE(), DATEADD(DAY, 30, GETDATE()), 'HOAT_DONG'),

('SILVER30', N'Ưu đãi hạng Bạc', N'Giảm 30.000đ cho khách đạt từ 3 lượt đặt trong 30 ngày',
 'CO_DINH', 30000, NULL, 150000, 'SILVER', 100, 0, 1,
 GETDATE(), DATEADD(DAY, 30, GETDATE()), 'HOAT_DONG'),

('GOLD50', N'Ưu đãi hạng Vàng', N'Giảm 50.000đ cho khách đạt từ 6 lượt đặt trong 30 ngày',
 'CO_DINH', 50000, NULL, 200000, 'GOLD', 100, 0, 1,
 GETDATE(), DATEADD(DAY, 30, GETDATE()), 'HOAT_DONG'),

('DIAMOND100', N'Ưu đãi hạng Kim Cương', N'Giảm 100.000đ cho khách đạt từ 10 lượt đặt trong 30 ngày',
